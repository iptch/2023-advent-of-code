...
bool push_button(struct module **modules, int64_t modules_count, int64_t *dr_activations, int64_t button_pushes);
...
bool push_button(struct module **modules, int64_t modules_count, int64_t *dr_activations, int64_t button_pushes) {
    ...
    while (pulses) {
        if (strcmp(pulses->end->name, DR_MODULE) == 0) {
            if (pulses->end->inputs_on[0] && !dr_activations[0]) dr_activations[0] = button_pushes;
            if (pulses->end->inputs_on[1] && !dr_activations[1]) dr_activations[1] = button_pushes;
            if (pulses->end->inputs_on[2] && !dr_activations[2]) dr_activations[2] = button_pushes;
            if (pulses->end->inputs_on[3] && !dr_activations[3]) dr_activations[3] = button_pushes;
        }

        if (dr_activations[0] && dr_activations[1] && dr_activations[2] && dr_activations[3]) return true;
...
void do_work(char **lines, int line_count, const int *chars_per_line) {
    ...
    int64_t res[DR_INPUT_COUNT] = {0};
    int64_t counter = 1;
    while (!push_button(modules, line_count+1, res, counter++));
    printf("%lld", res[0] * res[1] * res[2] * res[3]);
