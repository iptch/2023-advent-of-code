...
struct brick_queue {size_t brick_index; struct brick_queue *next;};
...
int64_t support_count(struct brick **bricks, const bool *broken_bricks, size_t brick_index, size_t n);
int64_t disintegrate(struct brick **bricks, bool *broken_bricks, size_t brick_index, size_t n);
...
int64_t support_count(struct brick **bricks, const bool *broken_bricks, size_t brick_index, size_t n) {
    ...
    int64_t supports = 0;
    for (size_t i=0; i<n; i++) supports += !broken_bricks[i] && intersect(&lower_brick, bricks[i]);
    return supports;
}

int64_t disintegrate(struct brick **bricks, bool *broken_bricks, size_t brick_index, size_t n) {
    struct brick_queue *q = (struct brick_queue *)calloc(1, sizeof(struct brick_queue));
    struct brick_queue *e = q;
    q->brick_index = brick_index;
    broken_bricks[brick_index] = true;

    int64_t res = -1;
    while (q) {
        struct brick *current = bricks[q->brick_index];
        res++;
        struct point start = {current->start->x, current->start->y, current->end->z+1};
        struct point end = {current->end->x, current->end->y, current->end->z+1};
        struct brick upper_brick = {&start, &end};
        for (size_t i=0; i<n; i++) {
            if (!broken_bricks[i] && intersect(&upper_brick, bricks[i]) && support_count(bricks, broken_bricks, i, n) == 0) {
                broken_bricks[i] = true;
                struct brick_queue *new = (struct brick_queue *)calloc(1, sizeof(struct brick_queue));
                new->brick_index = i;
                e->next = new;
                e = new;
            }
...
void do_work(char **lines, int line_count, const int *chars_per_line) {
    ...
    for (size_t i=0; i<line_count; i++) res += disintegrate(bricks, calloc(line_count, sizeof(bool)), i, line_count);
