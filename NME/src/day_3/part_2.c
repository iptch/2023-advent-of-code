#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <stdbool.h>
#include <ctype.h>
#include "aoc_utils/aoc_utils.h"

#define INPUT_FILE "input.txt"

struct number {
    int start;
    int end;
    int x;
    int val;
};

void do_work(char **lines, int line_count, const int *chars_per_line);
int scan(char **lines, int m, int n, int i, int j);

int main(int argc, char *argv[]) {
    execute_on_input(INPUT_FILE, &do_work);
    return 0;
}

void do_work(char **lines, int line_count, const int *chars_per_line) {
    int res = 0;
    for (int i=0; i<line_count; i++) {
        for (int j=0; j<chars_per_line[i]; j++) {
            char c = lines[i][j];
            if (c == '*') {
                res += scan(lines, line_count, chars_per_line[j], i, j);
            }
        }
    }
    printf("%d\n", res);
}

int scan(char **lines, int m, int n, int i, int j) {
    int num_nums = 0;
    struct number ns[10];
    for (int a=-1; a<=1; a++) {
        for (int b=-1; b<=1; b++) {
            int x = i+a;
            int y = j+b;
            if (x >= 0 && x < n && y >= 0 && y < m) {
                if (isdigit(lines[x][y])) {
                    int start = y;
                    int end = y;
                    while (start-1 >= 0 && isdigit(lines[x][start-1])) {
                        start -= 1;
                    }
                    while (end+1 < n && isdigit(lines[x][end+1])) {
                        end += 1;
                    }
                    int value = 0;
                    int radix = 1;
                    for (int u=end; u>=start; u--) {
                        value += radix * (lines[x][u] - '0');
                        radix *= 10;
                    }
                    struct number n1;
                    n1.start = start;
                    n1.end = end;
                    n1.x = x;
                    n1.val = value;

                    bool has_num = false;
                    for (int k=0; k<num_nums; k++) {
                        struct number other_n = ns[k];
                        if (other_n.start == n1.start && other_n.end == n1.end && other_n.x == n1.x && other_n.val == n1.val) {
                            has_num = true;
                        }
                    }
                    if (!has_num) {
                        ns[num_nums++] = n1;
                    }
                }
            }
        }
    }
    if (num_nums == 2) {
        return ns[0].val * ns[1].val;
    }
    return 0;
}
