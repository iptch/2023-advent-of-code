...
int shortest_path[GRID_SIZE][GRID_SIZE][STRAIGHT_LINE][DIRECTION];
...
int compute_best() {
    ...
    for (int k=0; k<STRAIGHT_LINE; k++) {
        for (int l=0; l<DIRECTION; l++) {
            res = min(res, shortest_path[GRID_SIZE-1][GRID_SIZE-1][k][l]);
        }
    }
    return res;
}
...
struct min_queue *best(int val, struct min_queue *q, int **grid, int n, int m) {
    ...
    // go up
    if (q->d != DOWN && !(q->d == UP && q->straight_line == 2) && q->x-1>=0) {
        res = push(res, val + grid_val, q->x-1, q->y, q->d == UP ? q->straight_line + 1 : 0, UP);
    }

    // go down
    if (q->d != UP && !(q->d == DOWN && q->straight_line == 2) && q->x+1<n) {
        res = push(res, val + grid_val, q->x+1, q->y, q->d == DOWN ? q->straight_line + 1 : 0, DOWN);
    }

    // go left
    if (q->d != RIGHT && !(q->d == LEFT && q->straight_line == 2) && q->y+1<m) {
        res = push(res, val + grid_val, q->x, q->y+1, q->d == LEFT ? q->straight_line + 1 : 0, LEFT);
    }

    // go right
    if (q->d != LEFT && !(q->d == RIGHT && q->straight_line == 2) && q->y-1>=0) {
        res = push(res, val + grid_val, q->x, q->y-1, q->d == RIGHT ? q->straight_line + 1 : 0, RIGHT);
    }
