...
@Slf4j
public class Day16_2 extends Puzzle {
    public Day16_2(PuzzleInputLoader puzzleInputLoader) {
        super(Day16_2.class, puzzleInputLoader);
    }
    ...
    void puzzle(BufferedReader puzzleInput) throws IOException {
        ...
        for (var beam : getInitialBeams()) {
            visitMap.clear();
            beams.clear();
            beams.add(beam);
            processBeams();
            int energizedTiles = visitMap.size();
            log.info("{} tiles energized with initial beam {} {} {}", energizedTiles, beam.coordinate().rowIdx(), beam.coordinate().colIdx(), beam.beamDirection());
            maxEnergizedTiles = Math.max(maxEnergizedTiles, energizedTiles);
        }
        log.info("The answer is {}", maxEnergizedTiles);
    }

    private void processBeams() {
        while (!beams.isEmpty()) {
            log.debug("{} beams bouncing...", beams.size());
    ...
    private List<Beam> getInitialBeams() {
        List<Beam> beams = new LinkedList<>();
        for (int colIdx = 0; colIdx < mapWidth; ++colIdx) {
            beams.add(new Beam(new Coordinate(0, colIdx), BeamDirection.DOWN));
            beams.add(new Beam(new Coordinate(mapHeight - 1, colIdx), BeamDirection.UP));
        }
        for (int rowIdx = 0; rowIdx < mapHeight; ++rowIdx) {
            beams.add(new Beam(new Coordinate(rowIdx, 0), BeamDirection.RIGHT));
            beams.add(new Beam(new Coordinate(rowIdx, mapWidth - 1), BeamDirection.LEFT));
        }
        return beams;
    }
}
