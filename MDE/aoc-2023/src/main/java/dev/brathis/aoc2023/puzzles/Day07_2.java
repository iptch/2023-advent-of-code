...
@Slf4j
public class Day07_2 extends Puzzle {
    public Day07_2(PuzzleInputLoader puzzleInputLoader) {
        super(Day07_2.class, puzzleInputLoader);
    }
    ...
    private class Hand implements Comparable<Hand> {
        ...
        private void determineType() {
            if (occurrenceMap.containsKey('J')) {
                determineTypeWithJokers();
            } else {
                determineTypeNoJokers();
            }
        }

        private void determineTypeWithJokers() {
            int numberOfJokers = occurrenceMap.get('J');
            this.type = switch (numberOfJokers) {
                case 5, 4: {
                ...
                case 2: {
                    yield switch (occurrenceMap.size()) {
                        case 2: {
                            yield HandType.FIVE_OF_A_KIND;
                        }
                        case 3: {
                        ...
                        default:
                            throw new RuntimeException("Unexpected occurrenceMap size: %d".formatted(occurrenceMap.size()));
                    };
                ...
                default:
                    throw new RuntimeException("Unexpected numberOfJokers: %d".formatted(numberOfJokers));
            };
        }

        private void determineTypeNoJokers() {
            if (isFiveOfAKind()) {
