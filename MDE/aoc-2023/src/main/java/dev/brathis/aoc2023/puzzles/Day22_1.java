...
@Slf4j
public class Day22_1 extends Puzzle {
    public Day22_1(PuzzleInputLoader puzzleInputLoader) {
        super(Day22_1.class, puzzleInputLoader);
    }
    ...
    private final PriorityQueue<Brick> fallenBricks = new PriorityQueue<>(Comparator
            .comparingInt(Brick::minZ)
            .thenComparingInt(Brick::maxZ)
            .thenComparingInt(Brick::minY)
            .thenComparingInt(Brick::maxY)
    ...
    void puzzle(BufferedReader puzzleInput) throws IOException {
        ...
        for (var brick : bricks) {
            ...
            log.info("Placing brick {} at z ({},{}), ", brick, lowerZ, upperZ);

            footprint.stream()
                    .map(floor::get)
                    .filter(floorTile -> floorTile.floorHeight == highestFloorLevel)
                    .forEach(floorTile -> {
                        log.info("brick {} relies on brick {}, level={}", brick.id, floorTile.brickId, floorTile.floorHeight);
            ...
            fallenBricks.add(new Brick(brick.id, brick.x1, brick.y1, lowerZ, brick.x2, brick.y2, upperZ));
        }

        log.info("fallen bricks: {}", fallenBricks);
        int bricksWhichCanBeDisintegrated = 0;
        for (var brick : fallenBricks) {
            long bricksRelyingExclusivelyOnThisBrick = relyingBricks.getOrDefault(brick.id, Set.of()).stream()
                    .map(reliedUponBricks::get)
                    .map(Set::size)
                    .filter(size -> size == 1)
                    .count();
            log.info("{} bricks rely exclusively on brick {}", bricksRelyingExclusivelyOnThisBrick, brick.id);
            if (bricksRelyingExclusivelyOnThisBrick == 0) {
                log.info("brick {} can be disintegrated", brick.id);
                ++bricksWhichCanBeDisintegrated;
            } else {
                log.info("brick {} CANNOT be disintegrated", brick.id);
            }
        }
        log.info("The answer is {}", bricksWhichCanBeDisintegrated);
    }
